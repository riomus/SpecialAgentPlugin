...
TArray<FMCPToolInfo> FPythonService::GetAvailableTools() const
{
	...
	// execute
	{
		...
		Tool.Description = TEXT("Run arbitrary Python via IPythonScriptPlugin synchronously on the game thread with full UE5 API access; the primary escape hatch when no purpose-built tool fits. Returns {success, stdout, stderr (traceback on error), execution_time (seconds)}. "
			...
			"Workflow: read the resource mcp://unreal/best_practices first (golden rules, modern subsystem map, units, gotchas); prefer the dedicated reflection/*, utility/*, log/* and other domain tools for supported operations; before running a draft, scan it with python/diff_against_deprecated and resolve symbols with python/inspect_class, python/search_symbol, python/get_function_signature. "
...
FMCPResponse FPythonService::HandleGetFunctionSignature(const FMCPRequest& Request)
{
	...
	auto Task = [SafeC, SafeM, TempFile]() -> TSharedPtr<FJsonObject>
	{
		const FString Wrap = FString::Printf(TEXT(
			...
			"    _doc_full = (_fn.__doc__ or '').strip()\n"
			"    _doc = (_doc_full.splitlines() or [''])[0][:400]\n"
			"    try:\n"
			...
			"    except Exception:\n"
			"        # Fallback: parse the UE-bound docstring.\n"
			"        # UE format: 'name(arg1, arg2=default, ...) -> ReturnType -- doc' on line 1,\n"
			"        # then '\\nX.method(...) -> ...' for class methods, plus an Args:/Returns: block.\n"
			"        _source = 'docstring'\n"
			"        _first = _doc\n"
			"        _m = re.match(r'^[^(]*\\(([^)]*)\\)\\s*(?:->\\s*([^\\-:]+?))?(?:\\s*--.*)?$', _first)\n"
			"        if _m:\n"
			"            _argstr = _m.group(1).strip()\n"
			"            _ret = (_m.group(2) or '').strip()\n"
			"            _returns = _ret\n"
			"            _sig = '(' + _argstr + ')' + (' -> ' + _ret if _ret else '')\n"
			"            _argtypes = {}\n"
			"            # Args:\\n    name (type): doc\n"
			"            for _a in re.finditer(r'^\\s+([A-Za-z_][\\w]*)\\s*\\(([^)]+)\\)\\s*:', _doc_full, re.M):\n"
			"                _argtypes[_a.group(1)] = _a.group(2).strip()\n"
			"            # Split on top-level commas only (don't break list/tuple defaults).\n"
			"            _args = []\n"
			"            _depth = 0\n"
			"            _start = 0\n"
			"            for _i, _ch in enumerate(_argstr):\n"
			"                if _ch in '([{': _depth += 1\n"
			"                elif _ch in ')]}': _depth -= 1\n"
			"                elif _ch == ',' and _depth == 0:\n"
			"                    _args.append(_argstr[_start:_i]); _start = _i + 1\n"
			"            if _argstr.strip():\n"
			"                _args.append(_argstr[_start:])\n"
			"            for _arg in _args:\n"
			"                _arg = _arg.strip()\n"
			"                if not _arg: continue\n"
			"                _eq = _arg.find('=')\n"
			"                if _eq >= 0:\n"
			"                    _name = _arg[:_eq].strip(); _default = _arg[_eq+1:].strip()\n"
			"                else:\n"
			"                    _name = _arg; _default = ''\n"
			"                _params.append({'name':_name,'kind':'POSITIONAL_OR_KEYWORD',"
			"'default':_default,'annotation':_argtypes.get(_name, '')})\n"
			"except Exception as _e:\n"
			"    _ok = False\n"
			"    _err = repr(_e)\n"
			"with open(r'%s','w',encoding='utf-8') as _f:\n"
			"    json.dump({'success':_ok,'error':_err,'class':_cn,'method':_mn,"
			"'signature':_sig,'params':_params,'return_annotation':_returns,"
			"'doc':_doc,'source':_source}, _f)\n"
...
FMCPResponse FPythonService::HandleListEnumValues(const FMCPRequest& Request)
{
	...
	auto Task = [Safe, TempFile]() -> TSharedPtr<FJsonObject>
	{
		const FString Wrap = FString::Printf(TEXT(
			...
			"    try:\n"
			"        for _m in list(_cls):\n"
			"            # unreal.EnumBase exposes the integer via .value (not int()).\n"
			"            # Try .value first, then fall back to int() for stdlib IntEnum.\n"
			"            _v = None\n"
			"            for _attr in ('value','numeric_value'):\n"
			"                if hasattr(_m, _attr):\n"
			"                    try: _v = int(getattr(_m, _attr)); break\n"
			"                    except Exception: pass\n"
			"            if _v is None:\n"
			"                try: _v = int(_m)\n"
			"                except Exception: _v = None\n"
			"            _values.append({'name': str(getattr(_m,'name',_m)),"
			" 'value': (_v if _v is not None else -1),"
			" 'has_value': _v is not None})\n"
